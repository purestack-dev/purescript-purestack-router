...
class RunAPI row list handlers | row -> handlers where
  runAPI :: Record handlers -> Route -> Request -> Effect Response

instance RunAPI row RowList.Nil () where
  runAPI _ _ _ = pure notFound
...
instance
  ...
  runAPI handlers route@{ path, verb } req =
    case parseRoute @t { path, verb } of
      Nothing -> runAPI @row @rest @restHandlers (Record.delete (Proxy @field) handlers) route req
...
run
  :: forall @row list handlers
   . RowToList row list
  => RunAPI row list handlers
  => Record handlers
  -> Request
  -> Effect Response
run handlers req = runAPI @row @list @handlers handlers (routeFromRequest req) req
